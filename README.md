...
data-migration-platform/
├── .github/workflows/
│   └── main.yml               # CI/CD GitHub Actions (4 jobs)
├── src/
│   ├── auth.py                # Authentification Managed Identity
│   ├── config.py              # Configuration centralisée
│   ├── db.py                  # Connexion SQL Azure AD
│   ├── core/
│   │   ├── extract.py         # Téléchargement CSV depuis Blob
│   │   ├── transform.py       # Nettoyage et validation
│   │   ├── load.py            # Insertion dans Azure SQL
│   │   ├── idempotency.py     # Déduplication par hash
│   │   └── exceptions.py      # ValidationError
│   └── worker/
│       ├── main.py            # Worker ETL principal
│       └── dlq_monitor.py     # Moniteur Dead Letter Queue
├── terraform/
│   ├── provider.tf            # Provider + backend remote
│   ├── containerapps.tf       # Worker + DLQ Monitor + KEDA
│   ├── servicebus.tf          # Namespace + Topic + Subscription
│   ├── adf.tf                 # ADF Pipeline + Trigger
│   ├── alerts.tf              # Alertes Azure Monitor
│   └── monitoring_dashboard.tf   # Dashboard
├── docs/
│   └── screenshots/
        ├── 01_dashboard_azure_monitor.png
│       ├── 02_github_actions_pipeline.png
│       ├── 03_service_bus_dlq.png
│       └── 04_container_apps_status.png
├── tests/
│   └── test_transform.py      # Tests unitaires
├── Dockerfile.worker          # Image Worker ETL
├── Dockerfile.dlq               # Image DLQ Monitor
├── send_bad_message.py        # Script de test
